package com.boshys.bteutils.render;
...
public class CustomParticleRenderer {
    ...
    private static void renderConnections(WorldRenderContext context, MatrixStack matrices, Vec3d cameraPos) {
        ...
        for (BoshysBTEUtils.MarkerConnection conn : BoshysBTEUtils.markerConnections) {
            ...
            // Draw line between markers with thickness
    ...
    private static void drawThickLine(VertexConsumer buffer, MatrixStack matrices,
        ...
        // Calculate second perpendicular vector (cross product)
        double perp2X = dy * perpZ - dz * perpY;
        double perp2Y = dz * perpX - dx * perpZ;
        double perp2Z = dx * perpY - dy * perpX;

        // Scale by half thickness
        float halfThickness = thickness * 0.05f; // Scale factor to make it visible
        ...
        // Draw additional offset lines for thickness effect
        for (int i = 0; i < 4; i++) {
